  ...
  export default function Home() {
    ...
    const fetchNotes = async () => {
      ...
      // Default Sort: Pinned notes first, then by date descending
    ...
    const handleDragOver = (e: React.DragEvent, note: Note) => {
    ...
    const handleDrop = (e: React.DragEvent, targetNote: Note) => {
      ...
      setNotes((prevNotes) => {
        ...
        const draggedIdx = newNotes.findIndex((n) => n.id === draggedNote.id)
          ...
          {notes.length === 0 ? (
            ...
            notes.map((note) => (
              <div
                ...
                style={{ 
                  cursor: draggedNote ? 'grabbing' : 'pointer',
                  opacity: draggedNote?.id === note.id ? 0.5 : 1,
                  transform: dragOverNote?.id === note.id ? 'scale(1.02)' : 'scale(1)',
                  boxShadow: dragOverNote?.id === note.id ? '0 0 0 2px var(--foreground, #000)' : '',
          ...
          <div className={styles.modalOverlay} onClick={() => setViewingNote(null)}>
            <div className={styles.modalContent} onClick={(e) => e.stopPropagation()}>
              <div className={styles.modalHeader}>
                ...
                <button onClick={() => setViewingNote(null)} className={styles.closeBtn}>
                  <div style={{ transform: 'rotate(45deg)', display: 'flex' }}>
                    <PlusIcon />
                  </div>
                  <span style={{ display: 'none' }}>Close</span>
              ...
              <div style={{ marginTop: '1rem', whiteSpace: 'pre-wrap', lineHeight: '1.6', overflowY: 'auto', maxHeight: '60vh' }} className="custom-scrollbar">
                {viewingNote.content}
              </div>
              
              <div style={{ marginTop: '2rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid rgba(0,0,0,0.1)', paddingTop: '1rem' }}>
                {viewingNote.tags ? (
                  <div className={styles.tags}>
                    {viewingNote.tags.split(',').filter(t => t.trim()).map((tag, index) => (
                      <span key={index} className={styles.tag}>
                        {tag.trim()}
                      </span>
                    ))}
                ...
                <span className={styles.date}>{formatDate(viewingNote.createdAt)}</span>
              </div>
            </div>
          </div>
        )}
